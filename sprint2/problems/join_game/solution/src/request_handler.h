...
namespace http_handler {
	...
	const std::string_view prefix = "/api/v1/maps"sv; //полчение списка карт
	...
	class ApiHandler {
	private:
		json::array LoadRoadsToJson(const model::Map* map) const;

		json::array LoadBuildingsToJson(const model::Map* map) const;

		json::array LoadOfficesToJson(const model::Map* map) const;
	public:
		...
		StringResponse HandlerApiHandler(const StringRequest req) const {
			...
			if (!uri.compare(0, api_post_join.size(), api_post_join)) {
				...
				else if (req.method() == http::verb::post && content_type == ContentType::APP_JSON) {//Для входа в игру 
					try {
				...
				if (auto content_type = req[boost::beast::http::field::content_type]; !content_type.empty()) {
					...
					json::array arr;
					/*auto t = req[boost::beast::http::field::content_type];*/
					for (auto& map : maps) {
						json::object obj;
				...
				if (req.method() != http::verb::get && req.method() != http::verb::head) {
					resp = MakeStringResponse(http::status::bad_request, invalid_method_error, req.version(), req.keep_alive(), ContentType::APP_JSON);
					resp.set(http::field::allow, "GET, HEAD"sv);
					return resp;
				}
				else if (content_type == ContentType::APP_JSON) {//Для входа в игру 
					...
					try {
						...
						for (const auto& dog : dogs) {
							...
							obj["players"s] = json::value{
								{std::to_string(dog.second->GetId()),
									json::object{{"pos"s, json::array(dog.second->GetPos().x, dog.second->GetPos().y)}},
												{"speed"s, json::array(dog.second->GetSpeed().x, dog.second->GetSpeed().y)}},
												{"dir"s, dir}
							};
						...
						if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_HEADER_MISSING) {
							...
							resp = MakeStringResponse(http::status::method_not_allowed, authorization_method_missing, req.version(), req.keep_alive(), ContentType::APP_JSON);
							resp.set(http::field::allow, "GET, HEAD"sv);

						}
						else if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_TOKEN_NOT_FOUND) {

							resp = MakeStringResponse(http::status::method_not_allowed, token_not_found, req.version(), req.keep_alive(), ContentType::APP_JSON);
							resp.set(http::field::allow, "GET, HEAD"sv);
						}
