...
namespace http_server {
	...
	inline void ReportError(beast::error_code ec, std::string_view what) {
		boost::json::value custom_data{ {key_code,ec.value()},
										{key_text, ec.message()},
										{key_where, what}
		};

		std::string msg{ key_error };
	...
	private:
		...
		void HandleRequest(HttpRequest&& request) override {
			...
			//Rvalue-ссылку на запрос. + Функцию, отправляющую ответ клиенту. 
			request_handler_(stream_.socket().remote_endpoint(), std::move(request), [self = this->shared_from_this()](auto&& response) {
