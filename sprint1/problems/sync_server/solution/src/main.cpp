...
StringResponse MakeStringResponse(http::status status, std::string_view body, unsigned http_version,
    bool keep_alive,
    std::string_view content_type = ContentType::TEXT_HTML, bool is_allow = true) {
...
StringResponse HandleRequest(StringRequest&& req) {
    ...
    const auto text_response = [&req](http::status status, std::string_view text, bool is_allow) {
        return MakeStringResponse(status, text, req.version(), req.keep_alive(), ContentType::TEXT_HTML, is_allow);
        };

    if (req.method() == http::verb::get || req.method() == http::verb::head) {
        // Здесь можно обработать запрос и сформировать ответ
        auto target = req.target();
        //std::string html_string = "<strong>Hello, " + std::string(target.substr(1)) + "</strong>";
        std::string html_string = "Hello, " + std::string(target.substr(1));
        return text_response(http::status::ok, html_string, true);
    }
    else {
        return text_response(http::status::method_not_allowed, "Invalid method"sv, false);
    }
...
int main() {
	...
	net::io_context ioc;// Контекст для выполнения синхронных и асинхронных операций ввода/вывода
    ...
    while (true) {
        //Для двустороннего обмена данными с удалённым компьютером применяется сокет.
