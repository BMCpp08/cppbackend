...
namespace json_loader {
	...
	model::Game LoadGame(const std::filesystem::path& json_path) {
		...
		double dog_retirement_time = 1.;
		if (auto it = value.as_object().if_contains(key_dog_retirement_time); it) {
			dog_retirement_time = it->as_double();
		}
		...
		if (auto f_maps = value.as_object().find(key_maps); f_maps != value.as_object().end()) {
			...
			for (const auto& json_map : arr) {
					...
					Map map(id, json_map.as_object().at(key_name).as_string().c_str(), dog_speed, bag_capacity, dog_retirement_time);
