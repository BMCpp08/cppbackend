...
namespace app {
	...
	enum RecordsErrorReason {
		INVALIDE_MAX_ITEM = 0u,
	};
	...
	public:
		virtual void OnTick(std::chrono::milliseconds timestamp) = 0;
	...
	public:
		...
		void RemovePlayer(const std::string& name, model::Map::Id map_id) {
			if (auto it = players_.find(std::pair<std::string, std::string>{name, * map_id}); it != players_.end()) {
				players_.erase(it);
			}
		}
	private:
