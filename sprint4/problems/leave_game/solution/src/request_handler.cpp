		...
		if (!uri.compare(0, api_post_join.size(), api_post_join)) {
			...
			if (req.method() != http::verb::post && req.method() != http::verb::get && req.method() != http::verb::head) {
				...
				resp = MakeStringResponse(http::status::method_not_allowed, 
					post_method_error, req.version(), req.keep_alive(), ContentType::APP_JSON);
				...
				try {
					...
					resp = MakeStringResponse(http::status::ok, json::serialize(obj), 
					...
					if (err.GetErrorReason() == app::INVALIDE_NAME) {//несуществующий id карты
						resp = MakeStringResponse(http::status::bad_request, json_invalid_name, 
							req.version(), req.keep_alive(), ContentType::APP_JSON);
					}
					else if (err.GetErrorReason() == app::INVALIDE_MAP) {//пустое имя игрока,
						resp = MakeStringResponse(http::status::not_found, json_not_found, 
							req.version(), req.keep_alive(), ContentType::APP_JSON);
					}
				}
				catch (...) {//Если при парсинге JSON или получении его свойств произошла ошибка
					resp = MakeStringResponse(http::status::bad_request, json_parse_error,
						req.version(), req.keep_alive(), ContentType::APP_JSON);
				}
			}
		}
			...
			if (req.method() != http::verb::get && req.method() != http::verb::head) {
				resp = MakeStringResponse(http::status::method_not_allowed, invalid_method_error, 
					req.version(), req.keep_alive(), ContentType::APP_JSON);
			...
			if (auto content_type = req[boost::beast::http::field::content_type]; !content_type.empty()) {
				resp = MakeStringResponse(http::status::bad_request, bad_request, req.version(), 
				...
				resp = MakeStringResponse(http::status::ok, json::serialize(arr), req.version(), 
					req.keep_alive(), ContentType::APP_JSON);
			}
				...
				resp = MakeStringResponse(http::status::ok, json::serialize(obj), req.version(), 
					req.keep_alive(), ContentType::APP_JSON);
			}
			else {
				resp = MakeStringResponse(http::status::not_found, json_not_found, req.version(), 
					req.keep_alive(), ContentType::APP_JSON);
			}
		}
			...
			if (req.method() != http::verb::get && req.method() != http::verb::head) {
				...
				try {
					...
					resp = MakeStringResponse(http::status::ok,	app_.GetPlayers(authorization),
						req.version(), req.keep_alive(), ContentType::APP_JSON);
				}
					...
					if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_HEADER_MISSING) {
						resp = MakeStringResponse(http::status::unauthorized, authorization_method_missing, 
							req.version(), req.keep_alive(), ContentType::APP_JSON);
					}
					else if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_TOKEN_NOT_FOUND) {
						resp = MakeStringResponse(http::status::unauthorized, token_not_found, req.version(), 
							req.keep_alive(), ContentType::APP_JSON);
					}
			...
			if (req.method() != http::verb::get && req.method() != http::verb::head) {
				...
				try {
					...
					resp = MakeStringResponse(http::status::ok,	app_.GetGameState(authorization), req.version(),
						req.keep_alive(), ContentType::APP_JSON);
				}
				catch (app::GameError<app::AuthorizationGameErrorReason> err) {
					...
					if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_TOKEN_NOT_FOUND) {
						...
						resp = MakeStringResponse(http::status::unauthorized, authorization_header_req, req.version(), 
							req.keep_alive(), ContentType::APP_JSON);
					}
				}
				catch (const std::exception& exc) {
			...
			if (req.method() != http::verb::post) {
				resp = MakeStringResponse(http::status::method_not_allowed, invalid_method_error, req.version(), 
					req.keep_alive(), ContentType::APP_JSON);
				...
				try {
					...
					resp = MakeStringResponse(http::status::ok,	app_.SetPlayerAction(authorization, req.body()),
						req.version(), req.keep_alive(), ContentType::APP_JSON);
				}
				catch (app::GameError<app::AuthorizationGameErrorReason> err) {
					...
					if (err.GetErrorReason() == app::AuthorizationGameErrorReason::AUTHORIZATION_TOKEN_NOT_FOUND) {
						...
						resp = MakeStringResponse(http::status::unauthorized, authorization_header_req, 
							req.version(), req.keep_alive(), ContentType::APP_JSON);
					}
				}
				catch (...) {//Если при парсинге JSON или получении его свойств произошла ошибка
					resp = MakeStringResponse(http::status::bad_request, failed_parse_action, 
						req.version(), req.keep_alive(), ContentType::APP_JSON);
				}
			}
			else {
				resp = MakeStringResponse(http::status::bad_request, invalid_content_type, 
					req.version(), req.keep_alive(), ContentType::APP_JSON);
			}
		}
			...
			if (api_ignore_list_.count(api_game_tick) && api_ignore_list_.at(api_game_tick)) {
				resp = MakeStringResponse(http::status::bad_request, bad_request_invalid_endpoint, 
					req.version(), req.keep_alive(), ContentType::APP_JSON);
			}
				...
				try {
					...
					resp = MakeStringResponse(http::status::ok,	app_.SetTimeDelta(req.body()), req.version(),
						req.keep_alive(), ContentType::APP_JSON);
				}
				catch (...) {
					resp = MakeStringResponse(http::status::bad_request, invalid_tick_req, req.version(), 
						req.keep_alive(), ContentType::APP_JSON);
				}
			}
		}
		else if (!uri.compare(0, api_game_records.size(), api_game_records)) {
			try {
				if (req.method() != http::verb::get) {
					resp = MakeStringResponse(http::status::method_not_allowed, invalid_method_error, req.version(),
						req.keep_alive(), ContentType::APP_JSON);

					resp.set(http::field::allow, REQ_GET);
				}
				else {
			...
			catch (app::GameError<app::RecordsErrorReason> err) {
				if (err.GetErrorReason() == app::RecordsErrorReason::INVALIDE_MAX_ITEM) {
